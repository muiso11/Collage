...
class kalkulator(tk.Tk):
    ...
    def membuatTombol(self):
        ...
        for penampung in btn_list:
            ...
            if penampung == '=':
                tk.Button(self, text='=', width=22, command=perintah).grid(
                    row=baris, column=kolom, columnspan=5)
            else:
                tk.Button(self, text=penampung, width=5, command=perintah).grid(
    ...
    def hitung(self, key):
        if key == '=':
            ...
            try:
                ...
                self.layar.insert(tk.END, "-> ERROORR....!!!!")
        elif key == 'C':
            self.layar.delete(0, tk.END)
        else:
            if self.penentu:
